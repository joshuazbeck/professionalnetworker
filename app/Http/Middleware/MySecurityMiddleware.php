...
class MySecurityMiddleware
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (!$request->is("/") && !$request->is('login') && !$request->is('register') && !$request->is('dologin') && !$request->is('users') && !$request->is('users/create') && !$request->is('jobrest/*') && !$request->is('profilerest/*') && !$request->is('jobrest') && session("loggedIn") != true)
        {
            return redirect('/');
        }
