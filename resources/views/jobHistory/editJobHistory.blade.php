        ...
        <div class="col" style="border-style: none; ">
            <div class="card"
                ...
                <div class="card-header">
                    <h5 class="mb-0" style="color: var(- -light); text-align: left;">Edit Job History</h5>
                </div>
                <!-- Here is the error with inserting into database. -->
                <form method="post" action="{{ route('jobHistory.update', $job->getJobID()) }}">
                    @csrf
                    <input type="hidden" name="_method" value="put" />
                    <input type="hidden" name="userID" value="{{ $job->getUserID() }}" />
                    <div class="card-body d-sm-flex flex-row justify-content-sm-center" style="background:var(--gray-dark);">
                        <div style="width: 75%; height: auto;">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="job-company" value="{{ $job->getCompanyName() }}" style="color:white; background:var(--gray-dark);">
                            </div>

                            <div class="form-group">
                                <label for="job-title"
                                       style="width: auto; min-width: 100%; max-width: 100%; height: auto; color: var(- -light);">Job Title:</label>
                                <input type="text" name="job-title" value="{{ $job->getJobTitle() }}" style="color:white; background:var(--gray-dark);">
                            </div>

                            <div class="form-group">
                                <label for="date-start"
                                       style="width: auto; min-width: 100%; max-width: 100%; height: auto; color: var(- -light);">Date Started Employment:</label>
                                <input type="date" name="date-start" value="{{ $job->getStartDate() }}" style="color:white; background:var(--gray-dark);">
                            </div>

                            <div class="form-group">
                                <label for="date-stop"
                                       style="width: auto; min-width: 100%; max-width: 100%; height: auto; color: var(- -light);">Date Ended Employment:</label>
                                <input type="date" name="date-stop" value="{{ $job->getStopDate() }}" style="color:white; background:var(--gray-dark);">
                            </div>

                            <div class="form-group">
                                <label for="job-desc"
                                       style="width: auto; min-width: 100%; max-width: 100%; height: auto; color: var(- -light);">Job Description:</label>
                                <textarea type="textarea" name="job-desc" value="" style="color:white; background:var(--gray-dark);">{{ $job->getDesc() }}</textarea>
                            </div>
